...
#
#   1. Nothing changed?            -> exit quietly.
#   2. Run the website and app tests; if anything fails, stop (nothing is pushed or deployed).
#   3. Commit and push to GitHub.
#   4. Website changed?            -> build and deploy klips.pro.
#   5. App version number changed? -> tag a release; GitHub builds and publishes the installers.
#
...
# --- checks: never push or deploy something broken ---
...
# --- deploy the website when it changed ---
...
# --- release new installers when the app version number changes ---
...
if [ -n "$VERSION" ] && ! git rev-parse -q --verify "refs/tags/v$VERSION" >/dev/null; then
  if git ls-remote --exit-code --tags origin "refs/tags/v$VERSION" >/dev/null 2>&1; then
    ...
    git tag "v$VERSION" && git push -q origin "v$VERSION" >>"$LOG" 2>&1 \
      && log "tagged v$VERSION; GitHub is building and publishing the installers"
  fi
fi
